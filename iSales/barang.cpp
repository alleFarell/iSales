...
adrBarang allocationBrg(infotypeB barang){ //MFA
...
void insertFirstChild(adrKategori P, infotypeB barang){ //MFA
...
void insertAfterChild(adrKategori P, adrBarang Prec, infotypeB barang){ //MDH
    adrBarang R = allocationBrg(barang);
    if (nextB(Prec) != NULL){
        nextB(R) = nextB(Prec);
        prevB(nextB(R)) = R;
...
void insertLastChild(adrKategori P, infotypeB barang){ //MFA
    adrBarang R = allocationBrg(barang);
    if(pointerBrg(P)!=NULL){
...
void deleteFirstChild(adrKategori &P, adrBarang &Q){ //MDH
    Q = pointerBrg(P);
...
void deleteAfterChild(adrKategori &P, adrBarang Prec, adrBarang &Q){ //MFA
     /*
...
void deleteLastChild (adrKategori &P,adrBarang&Q){ //MDH
...
void inputBarang(List &L, infotypeK kategori, infotypeB barang){ //MFA
...
void deleteBarang(List &L, infotypeB barang){ //MDH
...
void printBarangOnly(List L){ //MFA
    adrKategori P = First(L);
    adrBarang Q;
...
adrBarang searchChild(adrBarang P,string nama){ //MFA
   while(P != NULL){
...
adrBarang searchID(List L,infotypeB barang){ //MDH
    adrKategori P = First(L);
    adrBarang Q;
    while(P!= NULL){
        Q = pointerBrg(P);
